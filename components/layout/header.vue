<template>
  <header class="header">
    <div class="flex-wrap">
      ...
      <span>
        ...
        <v-popover v-if="fb.currentUser === null">
          <button class="icon" v-tooltip="'Entrar com'">
        ...
        <v-popover v-else>
          ...
          <template slot="popover">
            <div>
              <nuxt-link :to="'/settings'" v-close-popover>
                <button class="icon">
        ...
        <v-popover>
          <button class="icon" v-tooltip="'Mais'">
    ...
    <modal v-if="showExtensions" @close="showExtensions = false">
      ...
      <div slot="body">
        <p class="info">A extensão que simplifica o acesso ao Postwoman</p>
        <div>
          <a
          ...
          >
            <button class="icon">
              ...
              <span>Firefox</span>
              <span class="icon" v-if="hasFirefoxExtInstalled" v-tooltip="'Instalado'">
        ...
        <div>
          ...
          >
            <button class="icon">
              ...
              <span>Chrome</span>
              <span class="icon" v-if="hasChromeExtInstalled" v-tooltip="'Instalado'">
                <i class="material-icons">done</i>
              </span>
            </button>
          </a>
    ...
    <modal v-if="showShortcuts" @close="showShortcuts = false">
      ...
      <div slot="body">
        <div>
          <label>Enviar Requisição</label>
          <kbd>{{ getSpecialKey() }} G</kbd>
        </div>
        <div>
          <label>Salvar nas coleções</label>
          <kbd>{{ getSpecialKey() }} S</kbd>
        </div>
        <div>
          <label>Copiar link da Requisição</label>
          <kbd>{{ getSpecialKey() }} K</kbd>
        </div>
        <div>
          <label>Reiniciar Requisição</label>
...
export default {
  ...
  data() {
    return {
      // Once the PWA code is initialized, this holds a method that can be called to show the user the installation prompt.
  ...
  mounted() {
    ...
    ;(async () => {
      ...
      if (!cookiesAllowed) {
        this.$toast.show("Nós usamos cookies", {
          icon: "info",
          duration: 5000,
      ...
      setTimeout(() => {
        if (!hasExtensionInstalled()) {
          this.$toast.show("Obtenha a extensão do Postwoman!", {
            icon: "extension",
            duration: 5000,
            theme: "toasted-primary",
            action: [
    ...
    nativeShare() {
      if (navigator.share) {
        navigator
          .share({
            title: "Postwoman",
            text: "Postwoman • A free, fast and beautiful API request builder - Web alternative to Postman - Helps you create requests faster, saving precious time on development.",
