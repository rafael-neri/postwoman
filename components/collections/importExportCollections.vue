  ...
  <modal v-if="show" @close="hideModal">
    <div slot="header">
      <ul>
        <li>
          <div class="flex-wrap">
            <h3 class="title">Importar / Exportar Coleções</h3>
            <div>
          ...
          <div class="flex-wrap">
            <span
              v-tooltip="{
                content: !fb.currentUser ? 'Autenticar primeiro' : 'Substituir atual',
              }"
            >
              <button :disabled="!fb.currentUser" class="icon" @click="syncCollections">
                <i class="material-icons">folder_shared</i>
                <span>import_from_sync</span>
              </button>
            </span>
            <button
              class="icon"
              @click="openDialogChooseFileToReplaceWith"
              v-tooltip="replace_current"
            >
              <i class="material-icons">create_new_folder</i>
              <span>replace_json</span>
              <input
                type="file"
    ...
    <div slot="footer">
      <div class="flex-wrap">
        ...
        <span>
          ...
          <button class="icon primary" @click="exportJSON" v-tooltip="download_file">export</button>
        </span>
      </div>
    </div>
  </modal>
...
export default {
  ...
  methods: {
    ...
    fileImported() {
      this.$toast.info("file_imported", {
        icon: "folder_shared",
      })
    },
    failedImport() {
      this.$toast.error("import_failed", {
        icon: "error",
      })
    },
